...
main_food_list = [
        ...
        {"item": "Family Share Meal", "price": 50, "stock": 3, "quantity": 0},
    ]

food_list = []
cart = []
running_price = 0
valid_orders = ['chicken Kebab', "lamb Kebab", "mixed meat kebab", "chicken on chips", "lamb on chips", "mixed meat on chips", "family share meal", "no"]
...
while True:
    ...
    want_food = string_check("Please write the name of which food you would like?",valid_orders)
    if want_food == "No":
    ...
    for item in food_list:
        if item['item'].lower() == want_food.lower():
            selected_item = item
